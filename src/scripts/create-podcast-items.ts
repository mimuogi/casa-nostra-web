			...
			.then((data) => {
				try {
					...
					const episodes: PodcastEpisode[] = Array.from(items).map((item) => {
						...
						const audioElement = item.getElementsByTagName("enclosure")[0];

						const title: string = titleElement
							? titleElement.textContent || ""
							: "";
						const podcastURL: string = linkElement
							? linkElement.textContent || ""
							: "";
						const pubDate: string = dateElement
							? dateElement.textContent || ""
							: "";
						const duration: string = durationElement
							? durationElement.textContent || ""
							: "";
						const season: number =
							seasonElement && seasonElement.textContent
								? parseInt(seasonElement.textContent) || 0
								: 0;
						const number: number =
							episodeElement && episodeElement.textContent
								? parseInt(episodeElement.textContent) || 0
								: 0;
						const description: string = descriptionElement
							? descriptionElement.textContent || ""
							: "";
						const imageUrl: string = imageElement
							? imageElement.getAttribute("href") || ""
							: "";
						const type: string = typeElement
							? typeElement.textContent || ""
							: "";
						const audioURL: string = audioElement
							? audioElement.getAttribute("url") || ""
							: "";
